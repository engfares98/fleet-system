...
export default function Dashboard() {
  ...
  const handleSubmit = async () => {
    ...
    if (activeTab === 'vehicles') {
      if (!formData.plate_number || !formData.type) { alert('رقم اللوحة ونوع المركبة مطلوبان'); return }
      ...
      table = 'vehicles'
      data = { plate_number: formData.plate_number, type: formData.type, model: formData.model || '', year: formData.year ? parseInt(formData.year) : null, status: formData.status || 'active', image: imageUrl, istimara_pdf: istimaraUrl, insurance_pdf: insuranceUrl }
    } else if (activeTab === 'drivers') {
      if (!formData.name || !formData.license_number) { alert('الاسم ورقم الرخصة مطلوبان'); return }
      let imageUrl = editMode ? editItem?.image : null
      ...
      table = 'drivers'
      data = { name: formData.name, license_number: formData.license_number, phone: formData.phone || '', status: formData.status || 'active', image: imageUrl, license_pdf: licenseUrl, iqama_pdf: iqamaUrl }
    } else if (activeTab === 'maintenance') {
      if (!formData.vehicle_id || !formData.maintenance_type || !formData.maintenance_date) { alert('حقول مطلوبة ناقصة'); return }
      table = 'maintenance'
      ...
      if (!formData.vehicle_id || !formData.date) { alert('حقول مطلوبة ناقصة'); return }
      table = 'fuel_logs'
      ...
      if (!formData.vehicle_id || !formData.incident_date) { alert('حقول مطلوبة ناقصة'); return }
      table = 'incidents'
      ...
      if (!formData.vehicle_id || !formData.violation_date) { alert('حقول مطلوبة ناقصة'); return }
      table = 'violations'
      ...
      if (!formData.email) { alert('البريد الإلكتروني مطلوب'); return }
      table = 'users'
  ...
  const getPlateDisplay = (item) => {
    if (item.plate_letters && item.plate_numbers) return `${item.plate_letters} ${item.plate_numbers}`
  ...
  const renderTable = () => {
    ...
    if (activeTab === 'vehicles') {
      ...
      columns = [
        ...
        { key: 'image', label: 'الصورة', isImage: true }
      ]
    } else if (activeTab === 'drivers') {
      data = drivers
      tableName = 'drivers'
              ...
              <tr key={item.id} className="hover:bg-gray-50">
                {columns.map(col => (
                  <td key={col.key} className="px-4 py-3 text-sm text-gray-700">
                    {col.isImage ? (
                      getValue(item, col.key) && getValue(item, col.key) !== '-' ? (
    ...
    if (activeTab === 'vehicles') {
      return (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">صورة المركبة</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">الاستمارة (PDF)</label>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">التأمين (PDF)</label>
      ...
      return (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">صورة السائق</label>
            <input type="file" accept="image/*" onChange={e => setFormData({...formData, imageFile: e.target.files[0]})} className="w-full px-3 py-2 border rounded-lg text-sm" />
          </div>
        </>
      )
            ...
            {formData.vehicle_id ? (
              <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex-1">
                  <div className="font-medium text-blue-900 text-sm">{vehicles.find(v => v.id === formData.vehicle_id)?.plate_number}</div>
                </div>
                <button type="button" onClick={() => setShowVehiclePicker(true)} className="px-3 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700">تغيير</button>
              </div>
            ) : (
              <button type="button" onClick={() => setShowVehiclePicker(true)} className="w-full px-3 py-2 border-2 border-dashed border-gray-300 rounded-lg hover:border-blue-400 hover:bg-blue-50 transition text-sm text-gray-600">اضغط لاختيار مركبة</button>
            )}
          </div>
  ...
  const regions = ['all', 'شمال', 'جنوب', 'شرق', 'غرب']
  const regionLabels = { all: '🗺️ الكل', 'شمال': '⬆️ شمال', 'جنوب': '⬇️ جنوب', 'شرق': '➡️ شرق', 'غرب': '⬅️ غرب' }
  ...
  return (
    <div className={`min-h-screen bg-gray-50 ${lang === 'ar' ? 'rtl' : 'ltr'}`} dir={lang === 'ar' ? 'rtl' : 'ltr'}>
          ...
          <div className="flex-1">
            {activeTab !== 'reports' ? (
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                ...
                <p className="text-gray-600">صفحة التقارير - سنضيفها في المرحلة التالية</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={handleSubmit} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium">{t[lang].save}</button>
              <button onClick={closeModal} className="flex-1 px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-medium">{t[lang].cancel}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showVehiclePicker && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <h3 className="text-xl font-bold mb-4">اختيار مركبة</h3>
            <input type="text" value={pickerSearch} onChange={e => setPickerSearch(e.target.value)} placeholder="ابحث برقم اللوحة أو النوع..." className="w-full px-4 py-2 border-2 border-gray-300 rounded-lg mb-4 focus:border-blue-500 focus:outline-none" autoFocus />
            <div className="max-h-96 overflow-y-auto space-y-2">
              {vehicles.filter(v => v.plate_number?.toLowerCase().includes(pickerSearch.toLowerCase()) || v.type?.toLowerCase().includes(pickerSearch.toLowerCase())).map(vehicle => (
                <button key={vehicle.id} onClick={() => { setFormData({...formData, vehicle_id: vehicle.id}); setShowVehiclePicker(false); setPickerSearch('') }} className="w-full text-start px-4 py-3 bg-gray-50 hover:bg-blue-50 rounded-lg transition border border-gray-200 hover:border-blue-300">
            ...
            <button onClick={() => { setShowVehiclePicker(false); setPickerSearch('') }} className="w-full mt-4 px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-medium">{t[lang].cancel}</button>
          </div>
        </div>
      )}
      ...
      {showDriverPicker && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <h3 className="text-xl font-bold mb-4">اختيار سائق</h3>
            <input type="text" value={pickerSearch} onChange={e => setPickerSearch(e.target.value)} placeholder="ابحث بالاسم أو رقم الرخصة..." className="w-full px-4 py-2 border-2 border-gray-300 rounded-lg mb-4 focus:border-blue-500 focus:outline-none" autoFocus />
            <div className="max-h-96 overflow-y-auto space-y-2">
              {drivers.filter(d => d.name?.toLowerCase().includes(pickerSearch.toLowerCase()) || d.license_number?.toLowerCase().includes(pickerSearch.toLowerCase())).map(driver => (
                <button key={driver.id} onClick={() => { setFormData({...formData, driver_id: driver.id}); setShowDriverPicker(false); setPickerSearch('') }} className="w-full text-start px-4 py-3 bg-gray-50 hover:bg-green-50 rounded-lg transition border border-gray-200 hover:border-green-300">
                  <div className="font-medium text-gray-900">{driver.name}</div>
                  <div className="text-sm text-gray-600">{driver.license_number} - {driver.phone || '-'}</div>
                </button>
              ))}
            </div>
            <button onClick={() => { setShowDriverPicker(false); setPickerSearch('') }} className="w-full mt-4 px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-medium">{t[lang].cancel}</button>
          </div>
        </div>
      )}
    </div>
